    ...
    <TabScreenLayout>
      <div className="px-5 pb-8 pt-8">
        <div className="flex items-center justify-between">
          <div>
            <button type="button" className="flex items-center gap-1 text-xs leading-[18px] text-text-subtle">
              {currentChild.name} · {currentChild.ageMonths}개월
              <img src={chevronDownIcon} alt="" className="size-[13px]" />
            </button>
            <p className="mt-1 text-[23px] leading-[34.5px] font-bold text-text-primary">리포트</p>
          </div>
          <button
            type="button"
            aria-label="녹음 세션 목록"
        ...
        <div className="mt-5 rounded-2xl bg-surface-dark p-5">
          <div className="flex items-center justify-between">
            <span className="text-[13px] leading-[19.5px] text-white/65">최신 세션 리포트 · 7월 18일</span>
            <span className="rounded-full bg-report-good/20 px-2 py-1 text-[10px] leading-[15px] text-report-badge-text">
              신뢰도 충분
            </span>
          </div>
          <p className="mt-6 text-[13px] leading-[19.5px] text-white/65">부모 발화 비율</p>
          <p className="mt-1 text-[37px] leading-[37px] font-bold text-primary">
            62<span className="text-lg leading-[18px]">%</span>
          </p>
          <button
            type="button"
            onClick={() => navigate("/report/detail")}
            className="mt-6 flex items-center gap-1 text-[13px] leading-[19.5px] font-semibold text-primary"
          >
            세션 리포트 보기
            ...
            <div className="flex gap-0.5 rounded-2xl bg-surface-cream p-0.5">
              {(["weekly", "monthly"] as const).map((p) => (
                <button
                  ...
                  onClick={() => setPeriod(p)}
                  className={`rounded-2xl px-[10px] py-1 text-[11px] leading-[16.5px] font-semibold ${
                    period === p
        ...
        <h2 className="mt-6 text-[17px] leading-[25.5px] font-semibold text-text-primary">이번 주 돌아보기</h2>
        <div className="mt-3 flex flex-col gap-2">
          {weeklyHighlights.map((item) => (
            <div key={item.title} className="flex gap-3 rounded-[20px] bg-surface-cream p-3">
              <span className="text-base">{item.emoji}</span>
              <div>
                <p className="text-sm leading-[21px] font-semibold text-text-primary">{item.title}</p>
                <p className="mt-0.5 text-xs leading-[18px] text-text-body">{item.body}</p>
              </div>
            </div>
          ))}
        </div>

        <p className="mt-5 border-t-[0.5px] border-border pt-3 text-center text-[11px] leading-[16px] text-text-subtle">
          참고 정보이며 발달 진단이 아니에요.
        </p>
      </div>
    </TabScreenLayout>
