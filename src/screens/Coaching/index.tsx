  ...
  return (
    <TabScreenLayout>
      <div className="px-5 pb-8 pt-8">
        <h1 className="text-[23px] leading-[34.5px] font-bold text-text-primary">오늘의 실천</h1>
        <p className="mt-1 text-[13px] leading-[19.5px] text-text-subtle">
          최신 리포트 기반 · {currentChild.name} ({currentChild.ageMonths}개월)
        </p>

        <div className="mt-4 flex gap-2">
          {relatedTags.map((tag) => (
            <span key={tag} className="rounded-full bg-surface-cream px-3 py-2 text-xs leading-[18px] text-text-body">
              {tag}
            </span>
          ))}
        </div>
        ...
        <div className="mt-4 rounded-2xl bg-surface-dark p-5">
          ...
          <p className="mt-2 text-[19px] font-semibold leading-[28px] text-text-inverse">
            {todayScript.primaryQuote}
          </p>
          <button
            type="button"
            onClick={() => navigate("/coaching/script")}
            className="mt-5 rounded-2xl bg-white/10 px-4 py-2 text-xs leading-[18px] font-semibold text-primary"
          >
            스크립트 전체 보기
          </button>
        </div>
        ...
        <div className="mt-4 rounded-2xl border-[0.5px] border-border bg-surface p-4">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-[15px] leading-[22.5px] font-semibold text-text-primary">{todayMission.title}</p>
              <p className="mt-1 text-[13px] leading-[19.5px] text-text-body">{todayMission.description}</p>
            </div>
            <button
              type="button"
              role="checkbox"
          ...
          <button
            type="button"
            onClick={() => setMissionDone((v) => !v)}
            className="mt-3 text-xs leading-[18px] font-semibold text-text-link"
          >
        ...
        <div className="mt-3 rounded-2xl bg-surface-cream p-4">
          <p className="text-[15px] leading-[22.5px] font-semibold text-text-primary">전후 비교</p>
          <p className="mt-2 text-[13px] leading-[20px] text-text-body">
